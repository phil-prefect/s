...
//	"expandrecord.json",
    ...
    if(argc<=1)
    {
	print_cubeerr("format error! should be %s envset_file [argv1] [argv2] ...\n");
	return -EINVAL;
    }
    ret=envset_func(argv[1]);
    if(ret<0)
    {
	print_cubeerr("Set cube proc's env failed!\n");
	return -EINVAL;
    }

    start_para.argc=argc-1;
    start_para.argv=Dalloc0(sizeof(char *)*start_para.argc,0);
   
    start_para.argv[0]=argv[0];
